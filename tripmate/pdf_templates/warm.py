"""Warm 模板：暖色休闲度假风——珊瑚橙主色 / 金黄强调 / 米白卡片底。

与 Classic 覆盖相同的 7 个章节、数据读取方式一致，版式重新设计：
暖色编号章节标题（基类 section 自动主题化）、照片日头条 + 米白正文卡、
边框式信息总览卡、金边高亮合计条、美食色块网格。
所有图片/封面均回退安全（基类积木失败返回 ""，调用方降级纯色条/文字卡）。
"""
...
class WarmTemplate(BaseTripTemplate):
    name = "warm"
    display_name = "Warm 暖阳度假"
    description = "珊瑚橙暖渐变封面 + 米白卡片正文 + 金黄点缀，松弛感的度假风旅行路书"
    scenes = "休闲度假、亲子家庭、美食漫游"

    # ---- 暖色主题配色 ----
    PRIMARY = colors.HexColor("#e76f51")       # 珊瑚橙主色
    PRIMARY_HEX = "#e76f51"
    ACCENT = colors.HexColor("#e9c46a")        # 金黄强调（日头条色条自动取 ACCENT_RGB）
    ACCENT_RGB = (233, 196, 106)
    SUCCESS = colors.HexColor("#2a9d8f")       # 暖调青绿（勾选标记）
    SUCCESS_HEX = "#2a9d8f"
    WARN = colors.HexColor("#c01c28")
    GRAY = colors.HexColor("#8c7b6b")          # 暖灰说明文字
    LIGHT = colors.HexColor("#fae7d5")         # 日程卡时段列暖浅底
    BG_LIGHT = colors.HexColor("#fdf6ec")      # 米白卡片底
    HAIRLINE = colors.HexColor("#eadbca")      # 浅暖灰细线
    COVER_TOP_RGB = (184, 70, 32)              # 封面渐变：深橙
    COVER_BOT_RGB = (231, 111, 81)             #          → 珊瑚
    COVER_GOLD_RGB = (253, 246, 236)           # 封面装饰线/大字：奶白
    ...
    def build_story(self, profile: TravelProfile, budget: dict) -> list:
        ...
        t.setStyle(TableStyle([
            ...
            ("LINEAFTER", (0, 0), (0, -1), 0.4, self.HAIRLINE),
            ("LINEAFTER", (2, 0), (2, -1), 0.4, self.HAIRLINE),
        ...
        # 合计行：米白底 + 金黄左竖条（暖风高亮条）
        tot = Table([[Paragraph(f"已勾选订单合计（交通往返 + 住宿）："
                                f"<font face=\"{bold_font_name()}\">约 {total_order:g} 元</font>",
                                st("order_total", 10))]], colWidths=[CONTENT_W])
        tot.setStyle(TableStyle([
            ("BACKGROUND", (0, 0), (-1, -1), self.BG_LIGHT),
            ("LINEBEFORE", (0, 0), (0, 0), 2.5, self.ACCENT),
            ("TOPPADDING", (0, 0), (-1, -1), 5), ("BOTTOMPADDING", (0, 0), (-1, -1), 5),
        ...
        # ---- 叁 逐日行程（照片日头条 / 主题色日头条 + 米白正文卡）----
        story.append(self.section("叁", "逐日行程"))
        story.append(Spacer(1, 5))
        if profile.draft:
            slot_st = lambda: st("slot", 9, bold=True, color=self.PRIMARY)  # noqa: E731
            for i, day in enumerate(profile.draft.days):
                ...
                if strip_path:
                    # 照片头条（178×30mm，金色边条）+ 米白正文卡，视觉上是一张卡
                    ...
                    t.setStyle(TableStyle([
                        ...
                        ("BACKGROUND", (1, 1), (1, -1), self.BG_LIGHT),
                        ("BOX", (0, 0), (-1, -1), 0.6, self.HAIRLINE),
        ...
        # ---- 伍 实景速览（基类 image_cell：边框/底色自动随主题）----
        if profile.images:
        ...
        if enriched:
            ...
            for h in enriched[:2]:
                ...
                card.setStyle(TableStyle([
                    ...
                    ("LINEBEFORE", (1, 0), (1, 0), 0.4, self.HAIRLINE),
                    ("VALIGN", (0, 0), (-1, -1), "TOP"),
                    ("TOPPADDING", (0, 0), (-1, -1), 5), ("BOTTOMPADDING", (0, 0), (-1, -1), 5),
        ...
        if warn_items:
            ...
            wt.setStyle(TableStyle([
                ("BACKGROUND", (0, 0), (-1, -1), self.BG_LIGHT),
                ("LINEBEFORE", (0, 0), (0, -1), 2.5, self.ACCENT),
                ("LINEBELOW", (0, 0), (-1, -2), 0.4, self.HAIRLINE),
