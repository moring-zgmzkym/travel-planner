...
# "将为您生成行程草案"类措辞为 2026-09-23 e2e 实测第 5 种漏接变体（新主模型
# deepseek-v4-flash 反思轮连续空回复 → llm 层重试后仍空 → nudge 总结轮被指令
# 禁止调工具，start_planning 丢失；nudge 总结用"接下来规划团队将为您生成行程草案"
# 许诺了启动却无任何"开始/启动"字样，兜底未命中导致团队从未启动、用户被晾着）。
_START_INTENT = re.compile(
    r"(?<![不别未勿再后没暂])(?:开始|启动)[^。！？]{0,8}规划"
    r"|规划团队[^。！？]{0,8}(?<![不别未勿再后没暂])(?:启动|开工|开始|出)"
    r"|(?<![不别未勿再后没暂])(?<!无法)(?:生成|制定|产出|呈现|输出|制作)[^。！？]{0,6}(?:行程|路书|方案|草案|计划)"
    r"|start_planning")
    ...
    async def handle_user_message(self, text: str) -> str:
        ...
        async with self.chatter_lock:
            ...
            try:
                ...
                try:
                    # 2026-09-23 e2e 实测（新主模型 deepseek-v4-flash 反思轮连续空回复）：
                    # 原指令"不要再次调用任何工具"把模型逼进两难——它明知该调
                    # start_planning 却禁令不敢调（审计日志内心独白实证），
                    # 规划静默死亡、用户被"稍后出方案"的空许诺晾着。
                    # 改为放行未完成的必要动作（点名两个前向工具），同时禁重复调
                    # 刚成功的同一工具防循环；仍不调则由下方 _START_INTENT 兜底。
                    reply = await asyncio.wait_for(
                        stream_chatter(self.chatter,
                                       "上一轮工具已执行成功，但你的总结回复生成失败了。请重新组织这一轮："
                                       "如果用户的请求还有未完成的必要动作（例如需求已齐备时调用 "
                                       "start_planning 启动规划，或对待确认草稿的修改意见调用 "
                                       "submit_draft_feedback 提交），请先通过工具调用完成该动作，"
                                       "然后用一两句自然的中文告诉用户当前进展；"
                                       "如果没有未完成的动作，直接回复即可。"
                                       "不要重复调用刚刚已经执行成功的同一个工具。",
