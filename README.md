...
| #14 成本控制 | 共享模型客户端 `total_usage()` 聚合，500K/次规划熔断（§2.3；较企划书 200K 提额：完整流程实测 179K、带波折 343K；每次 `start()` 重置计数，语义保持"单次规划"） |
...
- 聊天历史与画像快照持久化于 sessions/{sid}.json（服务重启后刷新页面自动恢复；运行中的团队任务与状态时间线不恢复）。多标签页同时打开同一会话时回复推送至最新连接；多用户并发隔离为二期。
